I want to create the FAQs for the Location pages. I already have a spa-faqs skill within this project, but I want the appraoch for the location FAQs to be abit different. I want to create a specific location-faq skill. The skill will being by asking me for input on which location to start with and I'll provide it with the relevant data within the project that relates to the location, for examples the spas which are within that location and some general intro text we display on the location pages and the day passes, faqs and treatments which belong to those spas. The skill should make sure we don't overlap with any of the spa FAQs. The location FAQs should target different keywords

One you've reviewed the inital data just like the spa-faqs skill, I'd like you to output location based keywords for me to research. I will go away and do the research and then I'll provide you with the CSV reports.
<example keyword CSV report>
Keyword Stats 2026-02-09 at 11_47_15
...
north lakes hotel spa day GBP 50 0% 0% High 93
north lakes hotel reviews GBP 50 0% 0% Low 13 0.03 0.47
north lakes hotel spa treatments GBP 50 0% 0% Medium 52
north lakes hotel afternoon tea GBP 50 0% 0% High 86 0.02 0.56
<example keyword CSV report>
This should be a maximum of 8 keywords to research. Then I want you to analysis the reports in a similar way to the spa-faqs skill, expect I want to target low competition keywords which have traffic between 10 and 100.

Once you have the selected keywords, I'd like you to generate 4 faqs for each location. The FAQs should be in the same format as the spa faqs in the /src/data/faqs/ directory. The text length and tone should be roughly the same. The styles should align. I want the location FAQs to be kept in a new folder called location-faqs which should sit in /src/data/

Any spa, treatment or day pass data within the FAQs such as prices, duration or facilities should be output from the data src using helper and SHOULD NOT be hardcoded. The existing FAQs already has a load of helpers which are used for this so they should be reused where possible.

Below is an example of a spa faqs file also review the src/data/faqs/README.md file as that has relevant information on how FAQs work.
