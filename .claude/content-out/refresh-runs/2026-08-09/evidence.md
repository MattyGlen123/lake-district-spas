...
- **Artifact:** `.claude/content-out/refresh-runs/2026-08-09/spa-15.html` — 11 fetched pages, bundled and trimmed to the quoted spans
  - 1,233 KB → 148 KB (**88% smaller**), verified by `trim-artifact.mjs`: **11/11 gate verdicts identical** to the full pages
  - all 11 fetches HTTP 200 — per-page logs in `spa-15-fetch-log.json`
- **Gate results:** `.claude/content-out/refresh-runs/2026-08-09/spa-15-gate-results.json` · **Checks:** `spa-15-checks.json`
...
- **Renames applied:** 11 (tier-1 auto-applies — the portal suffixes every package with its availability window)
