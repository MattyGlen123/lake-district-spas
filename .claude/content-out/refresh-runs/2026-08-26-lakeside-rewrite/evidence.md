...
- Artifact: `spa-9.json` — HTTP 200, 2,505 B raw, pretty-printed on save by `fetch-onejourney.mjs`
...
Removed (6): `lakeside-afternoon-tea-spa-day` £150 · `lakeside-seasonal-spa-day` £90 ·
...
## Per-pass grounding — 4/4 grounded, 0 flags

All four use `arithmetic: "pence"` (`price.amount` is pence), the same gate 1 case as the SSR
onejourney tier.
...
### `lakeside-fizz-and-float` — Fizz and Float — £39 (new entry)

> `"name": "Fizz and Float ",` … `"price": {` `"amount": 3900`

Gate: grounded. Item 18902. ℹ️ The source `name` carries a trailing space (`"Fizz and Float "`);
`packageName` stores the trimmed form. Gate 2 matches on the substring, so the quote is unaffected.
...
- **`spaDuration: 3`** for Dip & Dine, Fizz and Float and Signature Sanctuary is the source's own
...
- **`ageRestriction: 'All ages'`** is carried over from the previous entries. The API exposes no
  age policy (`gender: null` on all four), and it describes the venue rather than the package.

## Availability

`daysAvailable: 'Monday-Sunday'` was checked against the real timeslots endpoint
(`/340/spa-packages/<id>/<date>/timeslots?quantity=1`) across 14 days from 2026-08-27:

| item | result |
| --- | --- |
| 18904 Dip & Dine | slots on 11 of 14 days (10:00–20:00) |
| 18905 Express Escape | slots on 11 of 14 days (10:00–20:00) |
| 18912 Signature Sanctuary | slots on 12 of 14 days (10:00–17:30) |
| 18902 Fizz and Float | ⚠️ **0 slots on all 14 days** |

The first two days return nothing for every package, consistent with the 48-hour notice in the
terms. **Fizz and Float is listed and priced in the catalogue but has released no availability at
all in the next fortnight** — it is included here because it is a live, listed product, but it is
worth a human check before relying on it.

ℹ️ Node's `fetch` and `curl` get *different* bodies from the timeslots endpoint (`{"data":[]}` vs a
populated array) for the same URL and headers. The figures above are curl's, which is what the
pipeline uses.
...
`{ "runDate": "2026-08-26", "ok": true, report: [{ spaId: "9", fetched: true, passes: 4, violations: [] }] }` — exit 0, **0 violations**.
